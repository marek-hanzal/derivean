...
import {withCacheService}           from "../container/withCacheService";
import {withContainer}              from "../container/withContainer";
import {withUserService}            from "../container/withUserService";
...
export class RpcService implements IRpcService {
    static inject = [
        ...
        lazyOf(withCacheService.inject),
    ];
    ...
    public async handle(
        ...
        for (const [id, bulk] of Object.entries(bulks.data.bulk)) {
            try {
                ...
                response.set(id, {
                    data: await this.cacheService.cache(
