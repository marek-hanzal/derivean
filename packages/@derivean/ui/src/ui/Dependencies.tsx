...
export namespace Dependencies {
	export interface Props {
		graph: serviceBlueprintGraph.Result;
		blueprintId: string;
		...
		return (
			<div className={tvc(["flex", "flex-row", "flex-wrap", "gap-2", "items-center"])}>
				{dependencies.length > 0 ? (
					<Icon
						css={{ base: ["text-emerald-500"] }}
						icon={"icon-[charm--circle-tick]"}
					/>
				) : (
					<Icon
						css={{ base: ["text-amber-500"] }}
				...
				{dependencies.length > 0 ? (
					...
					<Tx
						css={{ base: ["text-amber-500", "font-bold"] }}
			...
			return (
				<div className={tvc(["flex", "flex-row", "gap-2", "items-center"])}>
					...
					{e.cyclePath.map((item) => (
						<Badge
							key={genId()}
							css={{ base: ["bg-red-200", "text-red-700", "border-red-500"] }}
						>
							{graph.getNodeData(item)}
						</Badge>
					))}
