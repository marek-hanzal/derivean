...
export const MoistureBiome: Biome = {
	...
	resolve({ color, source, base }) {
		...
		const isWaterTransition = isTransitionBiome && types.some((type) => type === DefaultTerrainLayers.DeepOcean.name || type === DefaultTerrainLayers.Ocean.name || type === DefaultTerrainLayers.ShallowWater.name);

		// Use a curve to intensify the effect - stronger at extremes
		// Reduced for transition zones to avoid seams
		const intensityFactor = isTransitionBiome ? Math.abs(moisture) ** 0.6 * 0.3 : Math.abs(moisture) ** 0.8 * 0.7;
		...
		if (moisture < -0.6) {
			...
			// Cold desert / badlands
				...
				if (types.some((type) => type === DefaultTerrainLayers.Lowland.name || type === DefaultTerrainLayers.Highland.name || type === DefaultTerrainLayers.Midland.name)) {
			...
			if (types.some((type) => type === DefaultTerrainLayers.Highland.name || type === DefaultTerrainLayers.Foothills.name) && moisture < -0.5) {
				// Gray rocky appearance for dry highlands
				newColor = [200, 15, 40, 1];
		...
		// Very wet environments (rainforests, swamps, etc.)
			...
			if (temperature > 0.2) {
				if (types.some((type) => type === DefaultTerrainLayers.Lowland.name || type === DefaultTerrainLayers.Grassland.name || type === DefaultTerrainLayers.Valley.name || type === DefaultTerrainLayers.Midland.name)) {
			...
			// Temperate rainforest / swamp
				...
				if (types.some((type) => type === DefaultTerrainLayers.Lowland.name || type === DefaultTerrainLayers.Valley.name || type === DefaultTerrainLayers.Wetland.name)) {
					// More natural swamp green (slightly muted)
					newColor = [135, 55, 22, 1];
					blendFactor = Math.min(0.7, (moisture - 0.6) * 3);

					// Complete replacement for swamplands
					if (moisture > 0.85 && types.includes(DefaultTerrainLayers.Wetland.name)) {
						shouldReplace = true;
					}
				}
			}
		...
		if (Math.abs(moisture) > 0.7 && types.some((type) => type === DefaultTerrainLayers.DeepOcean.name || type === DefaultTerrainLayers.Ocean.name || type === DefaultTerrainLayers.ShallowWater.name)) {
			if (moisture < 0) {
		...
		if (newColor && blendFactor > 0) {
			...
			const blendedH = (((originalH + hueDiff * blendFactor) % 360) + 360) % 360;

			return {
				color: HSLA([blendedH, originalS * (1 - blendFactor) + newS * blendFactor, originalL * (1 - blendFactor) + newL * blendFactor, originalA]),
				exclusive: false,
			};
		}
		...
		switch (true) {
			...
			case types.some((type) => type === DefaultTerrainLayers.DeepOcean.name || type === DefaultTerrainLayers.Ocean.name || type === DefaultTerrainLayers.ShallowWater.name):
				if (moisture < 0) {
					...
					h = Math.max(0, Math.min(360, h - moisture * intensityFactor));
			...
			case types.some((type) => type === DefaultTerrainLayers.Beach.name || type === DefaultTerrainLayers.Coast.name || type === DefaultTerrainLayers.Foam.name):
				if (moisture < 0) {
			...
			case types.some((type) => type === DefaultTerrainLayers.Grassland.name || type === DefaultTerrainLayers.Lowland.name || type === DefaultTerrainLayers.Valley.name || type === DefaultTerrainLayers.Midland.name):
				if (moisture < 0) {
			...
			case types.some((type) => type === DefaultTerrainLayers.Highland.name || type === DefaultTerrainLayers.Foothills.name): {
				...
				const highlandEarthHue = 35;
				const highlandTintFactor = isTransitionBiome ? 0.05 : 0.1;

				// Apply a more natural earth tone appearance
				h = h * (1 - highlandTintFactor) + highlandEarthHue * highlandTintFactor;
				...
				if (isTransitionBiome) {
					...
					const transitionBlendFactor = 0.8;

					h = h * (1 - transitionBlendFactor) + originalHighlandH * transitionBlendFactor;
					s = s * (1 - transitionBlendFactor) + originalHighlandS * transitionBlendFactor;
					l = l * (1 - transitionBlendFactor) + originalHighlandL * transitionBlendFactor;
				}
			...
			// Mountain areas
			case types.some((type) => type === DefaultTerrainLayers.Mountain.name || type === DefaultTerrainLayers.HighMountain.name): {
				// Less dominant rocky appearance with richer coloration
				const grayRockHue = 30;
				const baseGrayFactor = isTransitionBiome ? 0.1 : 0.15;
				...
				if (moisture < 0) {
					...
					const dryTintFactor = -moisture * 0.3 * intensityFactor;
					h = h * (1 - dryTintFactor) + dryTint * dryTintFactor;
					s = Math.max(0, Math.min(100, s + 20));
					l = Math.max(0, Math.min(100, l * 0.9));
				} else {
				...
				if (heightmap > 0.7) {
					...
					const highRockTint = 25;
					const highRockFactor = (heightmap - 0.7) * 0.6;
					h = h * (1 - highRockFactor) + highRockTint * highRockFactor;
					s = Math.max(0, Math.min(100, s + highRockFactor * 10));
					l = Math.max(0, Math.min(100, l * (1 - highRockFactor * 0.2)));
				} else {
					...
					s = Math.max(0, Math.min(100, s + blendToOriginal * 15));
				}

				// If this is a transition zone, blend back toward original
				if (isTransitionBiome) {
					const transitionBlendFactor = 0.7;
					h = h * (1 - transitionBlendFactor) + originalH * transitionBlendFactor;
					s = Math.max(0, Math.min(100, s * 0.9 + 10));
					l = Math.max(0, Math.min(100, l * 0.95 + 2));
				}
				break;
			}
			...
			case types.includes(DefaultTerrainLayers.MountainPeak.name): {
				...
				const peakRockHue = 30;
				const peakRockFactor = isTransitionBiome ? 0.1 : 0.2;
				...
				if (moisture < 0) {
					...
					l = Math.max(0, Math.min(100, l + snowFactor * 20));
					s = Math.max(0, Math.min(100, s - snowFactor * 15));
				...
				if (isTransitionBiome) {
					const transitionBlendFactor = 0.8;
					h = h * (1 - transitionBlendFactor) + originalPeakH * transitionBlendFactor;
					s = s * (1 - transitionBlendFactor) + originalPeakS * transitionBlendFactor;
					l = l * (1 - transitionBlendFactor) + originalPeakL * transitionBlendFactor;
				}
				break;
			}
			...
			default:
				...
				if (isTransitionBiome) {
					...
					const [originalTransH, originalTransS, originalTransL] = color.color;
