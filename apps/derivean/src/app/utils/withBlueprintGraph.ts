...
export const withBlueprintGraph = async ({ tx }: withBlueprintGraph.Props): Promise<withBlueprintGraph.Result> => {
	const graph = new DepGraph<string>({ circular: false });

	for await (const { id, name } of await tx.selectFrom("Blueprint").select(["id", "name"]).execute()) {
