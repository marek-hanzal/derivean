...
export const Dependencies: FC<Dependencies.Props> = ({ graph, blueprintId, mode = "dependencies", reverse = false }) => {
	try {
		const dependencies = ((list) => {
			...
			return list;
		})(mode === "dependants" ? graph.dependantsOf(blueprintId) : graph.dependenciesOf(blueprintId));
				...
				{dependencies.length > 0 ? (
					dependencies.map((item) => (
						<Badge
							key={genId()}
							css={{
								base: ["bg-emerald-200", "text-emerald-700", "border-emerald-500"],
							}}>
