...
import { ActionMenu, ActionModal, DeleteControl, Table, TrashIcon, Tx, useInvalidator, useTable, withColumn } from "@use-pico/client";
import { genId, toHumanNumber, type IdentitySchema } from "@use-pico/common";
import type { FC } from "react";
import { kysely } from "~/app/db/kysely";
import { ResourceIcon } from "~/app/icon/ResourceIcon";
import { BlueprintProductionResourceForm } from "~/app/root/BlueprintProductionResourceForm";
...
export const BlueprintProductionResourceTable: FC<BlueprintProductionResourceTable.Props> = ({ blueprintProductionId, table, ...props }) => {
									...
									return (
										<BlueprintProductionResourceForm
											...
											mutation={useMutation({
												async mutationFn(values) {
													return kysely.transaction().execute(async (tx) => {
														return tx.updateTable("Blueprint_Production_Resource").set(values).where("id", "=", data.id).returningAll().executeTakeFirstOrThrow();
													});
												},
												async onSuccess() {
													await invalidator();
