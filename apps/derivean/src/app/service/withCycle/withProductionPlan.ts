...
export const withProductionPlan = async ({ tx, userId, mapId }: withProductionPlan.Props) => {
	...
	for await (const { buildingId, productionId } of productionPlanQueue) {
		try {
			...
			if (inQueue) {
				await tx.updateTable("Building").set({ productionId: null }).where("id", "=", buildingId).execute();
			}
		} catch (e) {
			console.error(e);
	...
	for await (const { buildingId, recurringProductionId } of recurringProductionPlanQueue) {
		try {
			const { count: queueSize } = await tx
				.selectFrom("Production as p")
				.where("p.buildingId", "=", buildingId)
