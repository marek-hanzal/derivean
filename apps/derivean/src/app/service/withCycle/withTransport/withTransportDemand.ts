...
export const withTransportDemand = async ({ tx, userId, mapId }: withTransportDemand.Props) => {
	...
	const demands = await tx
		...
		.select(["d.id", "d.amount", "d.resourceId", "d.buildingId", "bp.name as building", "d.type", "r.name as resource"])
	...
	for await (const { id, amount, resourceId, resource, building, buildingId: targetId, type } of demands) {
		...
		const supplies = await tx
			...
			.select(["s.buildingId", "bp.name as building", "i.id as inventoryId", "i.amount as available"])
		...
		for await (const { inventoryId, building, buildingId: sourceId, available, path } of suppliers) {
			...
			if (!roadId) {
				console.info("\t\t\t\t-- No waypoint available for this transport (path too short?)");
				continue;
			}
		...
		await tx.updateTable("Demand").set({ amount: remaining }).where("id", "=", id).execute();
