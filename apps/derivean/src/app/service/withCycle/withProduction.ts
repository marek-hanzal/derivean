...
export const withProduction = async ({ tx, userId, mapId }: withProduction.Props) => {
	...
	const productionQueue = await tx
		...
		.select(["p.id", "p.buildingId", "bl.name as building", "p.cycle", "p.cycles", "bp.resourceId", "p.blueprintProductionId", "r.name as resource", "bp.amount"])
	...
	for await (const { id, buildingId, resourceId, cycle, cycles, amount, building, resource } of productionQueue) {
		...
		if (storageInventory && (storageInventory.limit > 0 ? storageInventory.amount + amount <= storageInventory.limit : true)) {
