...
export const withProductionQueue = async ({ tx, userId, mapId, buildingId, blueprintProductionId }: withProductionQueue.Props) => {
	const blueprintProduction = await tx
		...
		.select(["bp.cycles", "r.name as resource", "bp.amount", "bp.cycles", "b.name as blueprint"])
	...
	for await (const { resourceId, amount, passive } of requirements) {
		...
		if (inventory.amount < amount) {
			...
			if (transport) {
				console.info("\t\t\t-- Transport already on the way, waiting for it to arrive");
				continue;
			}
	...
	if (proceed) {
		...
		for await (const [id, amount] of update) {
			await tx.updateTable("Inventory").set("amount", amount).where("id", "=", id).execute();
		}
