...
export const Item: FC<Item.Props> = ({ mapId, userId, inventory }) => {
	...
	const toggleSupplyMutation = useMutation({
		async mutationFn({
			...
			return kysely.transaction().execute(async (tx) => {
				...
				return tx.insertInto("Supply").values({ id: genId(), buildingId, resourceId, mapId, userId }).execute();
			});
		},
		async onSuccess() {
			await invalidator();
		...
		async mutationFn({
			...
			return kysely.transaction().execute(async (tx) => {
				...
				if (inventory.limit > 0 && request <= 0) {
					toast.error(
						translator.text("Not enough space (inventory is full/all resources are already on the way) (toast)"),
					);
					return;
				}
	...
	const formTva = FormCss({ isLoading: form.formState.isLoading, isSubmitting: form.formState.isSubmitting }).slots;
		...
		<div
			className={tvc([
				...
				inventory.limit > 0 && inventory.amount >= inventory.limit
					? ["border-amber-400", "bg-amber-50", "hover:bg-amber-50", "hover:border-amber-600"]
					: undefined,
		...
		>
			<div className={"flex flex-row items-center justify-between"}>
				<div
					className={tvc([
						...
						"gap-2",
						"items-center",
						inventory.supplyId || inventory.demandId ? ["text-purple-600"] : undefined,
					])}
				>
				...
				{inventory.demandId ? (
					...
					>
						<form
							...
							onSubmit={onSubmit<DemandSchema>({ form, mutation: toggleDemandMutation })}
						>
							<FormError
			...
			{inventory.limit > 0 ? (
				<Progress
					...
					css={{ progress: inventory.amount >= inventory.limit ? ["bg-amber-500"] : undefined }}
				/>
			) : null}
		</div>
	);
