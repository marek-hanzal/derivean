...
import { Button, FormCss, FormError, FormInput, Icon, Modal, onSubmit, Progress, toast, TrashIcon, Tx, useInvalidator } from "@use-pico/client";
import { genId, toHumanNumber, translator, tvc, withFloatSchema } from "@use-pico/common";
...
export const Item: FC<Item.Props> = ({ mapId, userId, inventory }) => {
	...
	const toggleSupplyMutation = useMutation({
		async mutationFn({ buildingId, resourceId, supplyId, mapId, userId }: { buildingId: string; resourceId: string; mapId: string; userId: string; supplyId?: string | null }) {
	...
	const toggleDemandMutation = useMutation({
		async mutationFn({ buildingId, resourceId, demandId, mapId, userId, amount }: { buildingId: string; resourceId: string; demandId?: string | null; mapId: string; userId: string; amount: number }) {
			return kysely.transaction().execute(async (tx) => {
				if (demandId) {
				...
				if (inventory.limit > 0 && request <= 0) {
					toast.error(translator.text("Not enough space (inventory is full/all resources are already on the way) (toast)"));
					return;
				}
	...
	return (
		<div
			className={tvc([
				...
				"cursor-default",
				"hover:bg-slate-100",
				inventory.limit > 0 && inventory.amount >= inventory.limit ? ["border-amber-400", "bg-amber-50", "hover:bg-amber-50", "hover:border-amber-600"] : undefined,
				inventory.supplyId || inventory.demandId ? ["bg-purple-50", "border-purple-400", "hover:bg-purple-50", "hover:border-purple-400"] : undefined,
			])}>
			<div className={"flex flex-row items-center justify-between"}>
				<div className={tvc(["flex", "flex-row", "gap-2", "items-center", inventory.supplyId || inventory.demandId ? ["text-purple-600"] : undefined])}>
					<div className={"font-bold"}>{inventory.name}</div>
					{inventory.supplyId ? <Icon icon={SupplyIcon} /> : null}
					{inventory.demandId ? <Icon icon={DemandIcon} /> : null}
				</div>
				{toHumanNumber({ number: inventory.amount })} / {toHumanNumber({ number: inventory.limit })}
			</div>
			<div className={"flex flex-row gap-2 items-center justify-between"}>
				<Button
					...
					{inventory.supplyId ? <Tx label={"Cancel supply (label)"} /> : <Tx label={"Supply resource (label)"} />}
				</Button>
				<div className={tvc(["border-2", "border-purple-400", "rounded-md", "w-[64px]", "h-[64px]", "bg-contain", `bg-${inventory.resourceId}`])} />
				{inventory.demandId ? (
					<Button
			...
			{inventory.limit > 0 ? (
				<Progress
					...
					css={{
						progress: inventory.amount >= inventory.limit ? ["bg-amber-500"] : undefined,
					}}
				/>
			) : null}
		</div>
