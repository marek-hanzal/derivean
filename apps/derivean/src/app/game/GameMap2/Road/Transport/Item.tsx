...
import { ArrowRightIcon, Badge, Icon, LinkTo, Progress } from "@use-pico/client";
import { toHumanNumber, tvc } from "@use-pico/common";
import type { FC } from "react";
import type { TransportPanel } from "~/app/game/GameMap2/Road/Transport/TransportPanel";
...
export const Item: FC<Item.Props> = ({ transport }) => {
	...
	return (
		<div className={tvc(["flex", "flex-col", "gap-2", "rounded-md", "border", "border-slate-300", "p-2", "cursor-default", "hover:bg-slate-100"])}>
			<div className={"flex flex-row items-center justify-between"}>
				<div className={tvc(["flex", "flex-row", "gap-2", "items-center"])}>
					<div className={"font-bold"}>{transport.name}</div>
					<div className={"font-light"}>{toHumanNumber({ number: transport.progress })}%</div>
				</div>
