...
export const TemperatureBiome: Biome = {
	...
	resolve({ color, source, base }) {
		...
		if (source.moisture > 0.3) {
			...
			const isWaterType = [DefaultTerrainLayers.DeepOcean.name, DefaultTerrainLayers.Ocean.name, DefaultTerrainLayers.ShallowWater.name].includes(base.type as string);
