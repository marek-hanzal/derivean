...
import { z } from "zod";
import { ConstructionPanel } from "~/app/game/GameMap2/Construction/ConstructionPanel";

export const Route = createFileRoute("/$locale/map/$mapId/land/$landId/construction")({
	async loader({ context: { queryClient, kysely, session }, params: { mapId, landId } }) {
										...
										.select([
											...
											(eb) =>
												eb
													...
													.select((eb) => eb.fn.count<number>("bg.id").as("count"))
																		...
																		eb.not(
																			eb.exists(
																				eb
																					...
																					.where("l.mapId", "=", mapId)
																					.whereRef("b.blueprintId", "=", "bd.dependencyId"),
																			),
																		),
																	),
															),
										...
										.where(
											...
											tx
												...
												.where(
													"bc.blueprintId",
													"in",
													tx.selectFrom("Building as bg").innerJoin("Land as l", "l.id", "bg.landId").select("bg.blueprintId").where("bg.userId", "=", user.id).where("l.mapId", "=", mapId),
												),
										)
										.where("bl.id", "in", tx.selectFrom("Blueprint_Region as br").innerJoin("Land as l", "l.regionId", "br.regionId").select("br.blueprintId").where("l.id", "=", landId))
										.as("blueprint"),
								)
								.selectAll()
	...
	component() {
		...
		const { land } = useLoaderData({
			from: "/$locale/map/$mapId/land/$landId",
		});
