...
import { z } from "zod";
import { ProductionPanel } from "~/app/game/GameMap2/Production/ProductionPanel";

export const Route = createFileRoute("/$locale/map/$mapId/building/$buildingId/production/list")({
	async loader({ context: { queryClient, kysely, session }, params: { mapId, buildingId } }) {
		const user = await session();

		return {
			production: await queryClient.ensureQueryData({
				queryKey: ["GameMap", mapId, "building", buildingId, "production", "list"],
				async queryFn() {
					return kysely.transaction().execute(async (tx) => {
						return withList({
							select: tx
																...
																eb.not(
																	eb.exists(
																		eb
																			...
																			.select(eb.lit(1).as("one"))
																			.where("i.id", "in", eb.selectFrom("Building_Inventory").select("inventoryId").where("buildingId", "=", buildingId))
								...
								.where("bp.blueprintId", "=", tx.selectFrom("Building as b").select("b.blueprintId").where("b.id", "=", buildingId))
								.where((eb) => {
									return eb.not(
										eb.exists(
											eb
												...
												.where((eb) => {
													return eb.not(
														eb.exists(
															eb.selectFrom("Building as b").select("b.blueprintId").where("b.userId", "=", user.id).where("b.constructionId", "is", null).whereRef("b.blueprintId", "=", "bpd.blueprintId"),
														),
													);
												}),
										),
