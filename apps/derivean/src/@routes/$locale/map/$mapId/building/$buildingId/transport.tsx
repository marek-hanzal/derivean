...
import { z } from "zod";
import { TransportPanel } from "~/app/game/GameMap2/Building/Transport/TransportPanel";

export const Route = createFileRoute("/$locale/map/$mapId/building/$buildingId/transport")({
	async loader({ context: { queryClient, kysely }, params: { mapId, buildingId } }) {
		return {
			transport: await queryClient.ensureQueryData({
				queryKey: ["GameMap", mapId, "building", buildingId, "transport", "list"],
				queryFn: async () => {
					return kysely.transaction().execute(async (tx) => {
						return withList({
							select: tx
								...
								.select(["t.id", "r.name as resource", "source.id as sourceId", "bl.name as source", "t.amount"])
