...
import { Kysely, withBoolSchema, withJsonOutputSchema } from "@use-pico/common";
import { z } from "zod";
import { BuildingPanel } from "~/app/game/GameMap2/Building/BuildingPanel";
import { RequirementPanel } from "~/app/game/GameMap2/Construction/Requirement/RequirementPanel";

export const Route = createFileRoute("/$locale/map/$mapId/building/$buildingId/view")({
	async loader({ context: { queryClient, kysely, session }, params: { mapId, buildingId } }) {
		const user = await session();

		return {
			user,
			requirement: await queryClient.ensureQueryData({
				queryKey: ["GameMap", mapId, "building", buildingId, "construction", "requirement"],
				async queryFn() {
						...
						return withList({
							select: tx
								...
								.innerJoin("Building as bg", (eb) => {
									return eb.onRef("bg.blueprintId", "=", "br.blueprintId").on("bg.id", "=", buildingId);
								})
								...
								.select([
									...
									(eb) => {
										return eb
											...
											.select((eb) => eb.fn.count<number>("t.id").as("transport"))
									...
									(eb) => {
										return eb
											...
											.where("s.buildingId", "in", eb.selectFrom("Building_To_Building as btb").select("btb.linkId").where("btb.buildingId", "=", buildingId))
											.as("supply");
									},
									(eb) => {
										return eb
											...
											.select(["i.amount"])
											.where("i.id", "in", tx.selectFrom("Building_Inventory as bi").select("bi.inventoryId").where("bi.buildingId", "=", buildingId))
