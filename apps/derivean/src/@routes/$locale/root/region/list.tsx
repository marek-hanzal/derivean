...
export const Route = createFileRoute("/$locale/root/region/list")({
			...
			async queryFn() {
				return kysely.transaction().execute((tx) => {
					return withListCount({
						select: tx.selectFrom("Region as r").select(["r.id", "r.name", "r.probability", "r.limit"]).orderBy("r.name", "asc"),
						query({ select, where }) {
							let $select = select;
							...
							if (where?.fulltext) {
								...
								$select = $select.where((eb) => {
									return eb.or([eb("r.id", "like", fulltext), eb("r.name", "like", fulltext)]);
								});
							}
